...
# CSS
  ...
  *f12버튼으로 Elements 탭의 Computed 에서 show all을 체크하고 font-size 검색하면 픽셀단위로 확인 가능
  <br>*기본폰트크기(16px) 
  <br>*같은 태그 안에 있으면 형제 / 감싸고 있는 태그는 자식 / 감싸준 태그는 부모
  ...
  *image-map.net : 이미지 좌표값 확인할수있음
  ...
  ## 아이디 선택자
  - 아이디 선택자는 웹 페이지에 포함된 여러 요소 중에서 특정 아이디 이름을 가지는 요소만을 선택해 줍니다.
  - sytle태그에서 #아이디 {} 이 부분을 다른 단락태그에서 id속성의 값으로 아이디를 넣어주면 스타일 적용
   
  ## 클래스 선택자
  - 클래스 선택자는 특정 집단의 여러 요소를 한 번에 선택할 때 사용합니다.
  - sytle태그에서 .클래스 {} 이 부분을 다른 단락태그에서 class속성의 값으로 클래스를 넣어주면 스타일 적용
